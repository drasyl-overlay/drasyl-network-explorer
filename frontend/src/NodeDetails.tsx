...
const NodeDetails = () => {
    ...
    if (node) {
        return (
            <div className="mt-3 p-5 shadow rounded-sm bg-white divide-y overflow-x-auto overflow-y-auto" style={{maxWidth: 'calc(100vw - 1.5rem', maxHeight: 'calc(100vh - 6.5rem)'}}>
            ...
            <div className="mt-3 p-5 shadow rounded-sm bg-white divide-y overflow-x-auto overflow-y-auto" style={{maxWidth: 'calc(100vw - 1.5rem', maxHeight: 'calc(100vh - 6.5rem)'}}>
                <div className="grid grid-cols-3 gap-5">
                    <div className="text-center">
                        <p className="text-xs text-gray-600 uppercase">Nodes</p>
                        <p className="text-xl font-source oldstyle-nums">
                            {nodes.length}
                        </p>
                    </div>
                    <div className="text-center">
                        <p className="text-xs text-gray-600 uppercase">Super Peers</p>
                        <p className="text-xl font-source oldstyle-nums">
                            {nodes.filter(({superPeers}) => !superPeers).length}
                        </p>
                    </div>
                    <div className="text-center">
                        <p className="text-xs text-gray-600 uppercase">Clients</p>
                        <p className="text-xl font-source oldstyle-nums">
                            {nodes.filter(({superPeers}) => superPeers).length}
                        </p>
                    </div>
